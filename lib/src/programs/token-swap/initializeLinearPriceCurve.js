...
const initializeLinearPriceCurveTx = ({ tokenSwap, slopeNumerator, slopeDenominator, initialTokenAPriceNumerator, initialTokenAPriceDenominator, callerTokenBAccount, tokenSwapInfo, tokenA, tokenB, poolTokenDecimals, walletPubKey, connection, initialTokenBLiquidity, } = {}, { callerTokenBAccountOwner, adminAccountOwner, } = {}) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { tokenIx, tokenMint: poolTokenMint } = yield (0, utils_1.generateTokenMintInstructions)(connection, walletPubKey, expectedSwapAuthorityPDA, null, poolTokenDecimals, web3_js_1.Keypair.generate());
    // get token account create instructions for swap pda
    const { tokenAccount: tokenATokenAccount, accountIx: createTokenATokenAccountIx, } = yield (0, utils_1.generateCreateTokenAccountInstructions)(connection, walletPubKey, tokenA, expectedSwapAuthorityPDA, web3_js_1.Keypair.generate());
    const { tokenAccount: tokenBTokenAccount, accountIx: createTokenBTokenAccountIx, } = yield (0, utils_1.generateCreateTokenAccountInstructions)(connection, walletPubKey, tokenB, expectedSwapAuthorityPDA, web3_js_1.Keypair.generate());
    ...
    const { tokenAccount: destinationAccount, accountIx: createDestinationAccountIx, } = yield (0, utils_1.generateCreateTokenAccountInstructions)(connection, walletPubKey, poolTokenMint.publicKey, adminAccountOwner ? adminAccountOwner : walletPubKey, web3_js_1.Keypair.generate());
